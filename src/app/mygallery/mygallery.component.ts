...
export class MygalleryComponent {
  ...
  fetchImages(): void {
    this.dataService.recieveApiRequest('getImages').subscribe(
      (response: any) => {
        ...
        // Convert selectedUserId to a string and filter images if selectedUserId is set
        const selectedUserIdStr = this.selectedUserId.toString();
        if (selectedUserIdStr) {
          this.images = this.images.filter(image => image.user_id.toString() === selectedUserIdStr);
        }
      },
      (error) => {
  ...
  showAllImages(): void {
    this.userId = ''; // Reset userId to fetch all images
    this.fetchImages(); // Fetch images without user filtering
  }

selectedUserId: string = ''; // Store the selected userId

  setUserId(userId: string): void {
    this.selectedUserId = userId.toString(); // Ensure selectedUserId is a string
